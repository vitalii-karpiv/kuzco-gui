...
interface GroupImageManagerProps {
  ...
  /** Bumped by the parent (e.g. after linking laptop images) to force a reload. */
  refreshKey?: number;
}
...
export function GroupImageManager({
  ...
  async function reloadImages() {
    setImages(await imageService.listByGroup(groupId));
  }
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        await imageService.uploadToGroup(groupId, file);
      }
      await reloadImages();
    } catch (err) {
  ...
  async function remove(image: Image) {
    ...
    try {
      ...
      if (group.imageUrl === image.s3Url) {
        onChange(await laptopGroupService.update({ id: groupId, imageUrl: "" }));
      }
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
  ...
  async function setMain(image: Image) {
    ...
    try {
      onChange(
        await laptopGroupService.update({ id: groupId, imageUrl: image.s3Url }),
      );
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
